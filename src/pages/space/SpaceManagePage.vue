<template>
  <div id="spaceManagePage" class="page-shell">
    <section class="page-toolbar space-manage-toolbar">
      <div class="page-toolbar-main space-manage-toolbar-main">
        <div class="space-manage-title">
          <p class="space-manage-kicker">Space Manager</p>
          <h2 class="page-title">空间管理</h2>
          <p class="page-subtitle">{{ toolbarSubtitle }}</p>
        </div>
        <a-space class="page-toolbar-actions space-manage-actions">
          <a-button class="space-action-btn" type="primary" href="/add_space" target="_blank">
            + 创建空间
          </a-button>
          <a-button class="space-action-btn space-action-btn--ghost" type="primary" ghost href="/my_space">
            进入我的空间
          </a-button>
        </a-space>
      </div>

      <div class="mini-kpi-row space-kpi-row" aria-label="空间概览">
        <span class="mini-kpi">
          <strong>{{ total }}</strong>
          <span>空间总量</span>
        </span>
    ...
    <section class="filter-toolbar compact-filter-toolbar space-filter-toolbar">
      <a-form
        layout="vertical"
        size="small"
        :model="searchParams"
    ...
    <section class="content-slab space-table-stage">
...
const highLevelCount = computed(() => {
  return dataList.value.filter((item) => Number(item.spaceLevel ?? 0) > 0).length
})
const toolbarSubtitle = computed(() => {
  if (activeFilterCount.value > 0) {
    return `当前已启用 ${activeFilterCount.value} 项筛选条件`
  }
  return '筛选并维护你的个人与团队空间'
})
...
#spaceManagePage .space-manage-toolbar {
  position: relative;
  overflow: hidden;
  border-color: #c8d7e7;
  background:
...
#spaceManagePage .space-manage-toolbar::after {
  content: '';
  position: absolute;
  inset: 0;
...
#spaceManagePage .space-manage-toolbar-main {
  position: relative;
  z-index: 1;
}

#spaceManagePage .space-manage-title {
  display: grid;
  gap: 4px;
}

#spaceManagePage .space-manage-kicker {
  margin: 0;
  width: fit-content;
  min-height: 22px;
  border-radius: 999px;
...
#spaceManagePage .space-manage-actions {
  position: relative;
  z-index: 1;
}

#spaceManagePage .space-action-btn {
  min-height: 30px;
  border-radius: 999px;
  border-color: #c8d8e9;
...
#spaceManagePage .space-action-btn:hover,
#spaceManagePage .space-action-btn:focus {
  border-color: #abc0d7;
  background: #e4eef8;
  color: #1f3852;
}

#spaceManagePage .space-action-btn--ghost {
  border-color: #d1deea;
  background: rgb(255 255 255 / 78%);
  color: #526a84;
}

#spaceManagePage .space-kpi-row {
  position: relative;
  z-index: 1;
  margin-top: 10px;
  gap: 8px;
...
#spaceManagePage .space-kpi-row .mini-kpi {
  min-height: 28px;
  border-radius: 10px;
}

#spaceManagePage .space-filter-toolbar {
  border-color: #ccdbea;
  background:
...
#spaceManagePage .space-type-segment {
  background: rgb(236 243 251 / 80%);
  border-radius: 999px;
}

#spaceManagePage .density-switch {
  min-height: 30px;
  border-radius: 999px;
...
#spaceManagePage .density-btn:hover {
  color: #2f4d6c;
  background: #e6f0fa;
}

#spaceManagePage .density-btn.is-active {
  color: #203b56;
  background: #dfeaf6;
  font-weight: 600;
}

#spaceManagePage .density-btn:focus-visible {
  outline: 2px solid #6d8fb2;
  outline-offset: 2px;
}

#spaceManagePage .active-filter-strip {
  min-height: 34px;
...
#spaceManagePage .active-filter-strip::-webkit-scrollbar {
  display: none;
}

#spaceManagePage .active-filter-label {
  font-size: 11px;
  color: #60758d;
...
#spaceManagePage .space-table-stage {
  display: grid;
  gap: 8px;
  border-color: #cad9e8;
  background:
...
#spaceManagePage .space-table-head {
  display: grid;
  gap: 4px;
}

#spaceManagePage .result-tip {
  margin: 0;
  font-size: 12px;
  color: #5a728d;
}

#spaceManagePage .usage-ratio {
  margin: 2px 0 0;
  color: #60768e;
  font-size: 12px;
}
...
@media (max-width: 992px) {
  #spaceManagePage .compact-filter-actions {
    justify-content: flex-start;
  }
}
